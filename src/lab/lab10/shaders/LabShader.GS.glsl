...
void main()
{
	...
	for(int i=0; i<5; i++)
	{
		gl_Position = Projection * View * (gl_in[0].gl_Position + vec4(0, 0, i, 0));
		texture_coord = v_texture_coord[0];
		EmitVertex();

		gl_Position = Projection * View * (gl_in[1].gl_Position + vec4(0, 0, i, 0));
		texture_coord = v_texture_coord[1];
		EmitVertex();

		gl_Position = Projection * View * (gl_in[2].gl_Position + vec4(0, 0, i, 0));
