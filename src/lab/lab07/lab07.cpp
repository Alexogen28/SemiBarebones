...
void Lab07::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & model, const glm::vec3 &object_color)
{
    ...
    int location = glGetUniformLocation(shader->GetProgramID(), "point_light_positions");
    glUniform3fv(location, 10, glm::value_ptr(point_light_positions[0]));

    location = glGetUniformLocation(shader->GetProgramID(), "spot_light_directions");
    glUniform3fv(location, 10, glm::value_ptr(spot_light_directions[0]));

    location = glGetUniformLocation(shader->GetProgramID(), "point_light_colors");
    glUniform3fv(location, 10, glm::value_ptr(point_light_colors[0]));

    location = glGetUniformLocation(shader->GetProgramID(), "spot_light_colors");
    glUniform3fv(location, 10, glm::value_ptr(spot_light_colors[0]));

    location = glGetUniformLocation(shader->GetProgramID(), "spot_light_positions");
    glUniform3fv(location, 10, glm::value_ptr(spot_light_positions[0]));

    location = glGetUniformLocation(shader->GetProgramID(), "spot_light_angles");
    ...
    location = glGetUniformLocation(shader->GetProgramID(), "material_ka");
...
void Lab07::OnInputUpdate(float deltaTime, int mods)
{
...

    {
        ...
        glm::mat4 rotationOX = glm::rotate(glm::mat4(1), glm::radians(50 * deltaTime), glm::vec3(1, 0, 0));
        glm::mat4 rotationOX2 = glm::rotate(glm::mat4(1), glm::radians(50 * deltaTime), glm::vec3(-1, 0, 0));

        glm::mat4 rotationOZ = glm::rotate(glm::mat4(1), glm::radians(50 * deltaTime), glm::vec3(0, 0, 1));
        glm::mat4 rotationOZ2 = glm::rotate(glm::mat4(1), glm::radians(50 * deltaTime), glm::vec3(0, 0, -1));

        if (window->KeyHold(GLFW_KEY_C)) {

            light_direction = glm::mat3(rotationOX) * light_direction;
        }

        if (window->KeyHold(GLFW_KEY_V)) {

            light_direction = glm::mat3(rotationOX2) * light_direction;
        }


        if (window->KeyHold(GLFW_KEY_Z)) {

            light_direction = glm::mat3(rotationOZ) * light_direction;
        }


        if (window->KeyHold(GLFW_KEY_X)) {

            light_direction = glm::mat3(rotationOZ2) * light_direction;
        }
