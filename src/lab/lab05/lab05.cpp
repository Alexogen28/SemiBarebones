...
void Lab05::Update(float deltaTimeSeconds)
{
    ...
    //CUBE TRANSFORMATIONS GO HERE

    cube_angle += 1 + deltaTimeSeconds;

    if (isMoving) {
        moveY = moveY + deltaTimeSeconds;
        if (moveY > max_pos_y) {
            isMoving = false;
        }
    } else {
    ...
    viewport_space = transform2D::ViewportSpace(0, 0, resolution.x * 0.75f, resolution.y * 0.5f);
    ...
    viewport_space = transform2D::ViewportSpace(0, resolution.y * 0.5f, resolution.x * 0.75f, resolution.y * 0.5f);
    DrawObjects(camera, viewport_space);
...
void Lab05::DrawObjects(gfxc::Camera *camera, const transform2D::ViewportSpace & viewport_space)
{
    ...
    model *= transform3D::RotateOZ(glm::radians(cube_angle));
    model *= transform3D::Scale(glm::vec3(0.75f));
    RenderMesh(meshes["cube"], shaders["VertexColor"], model, view, projection);
    ...
    model *= transform3D::Translate(glm::vec3(0, 0.5f, 0));
    model *= transform3D::Scale(glm::vec3(0.75f));
    model += transform3D::Translate(0, moveY, 0);
    RenderMesh(meshes["cube"], shaders["VertexColor"], model, view, projection);
    ...
    //MOVING CUBE
    model = glm::mat4(1);
    model *= transform3D::Translate(glm::vec3(0, 0, 0));
    model *= transform3D::Scale(glm::vec3(0.5f));
    model *= transform3D::Translate(position);
    RenderMesh(meshes["cube"], shaders["VertexColor"], model, view, projection);
