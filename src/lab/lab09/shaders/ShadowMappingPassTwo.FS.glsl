...
vec3 SpotLight()
{
    ...
    if (spot_light > cos(cut_off)) {
        ...
        return ShadowFactor() * light_att_factor * light_intensity * PhongLight() + KA;
    }
...
float VolumetricIllumination()
{
    ...
    {
        if(IsIlluminated(point_position, 0))
        {
            illuminated_samples_count++;
        }
        point_position = point_position + ray_direction / sample_count;
    }
